...
def run():
    ...
    # A - fly in lake and week - 25%
    # B - fly in lake - 10%
    # C - fly in week - 15%

    # D - fly in chart * strength - 20%

    # E - fly in hatch in lake and week - 10%
    # F - fly in hatch in lake - 10%%
    # G - fly in hatch in week - 10%%

    #  array is
    #  fly id, fly name, bug id, A,B,C,D,E,F,G,total

    #step 1 put all flys into numpy
    # flys = Fly.objects.values_list('id','name','bug').filter(gender='male').order_by('id')
    flys = Fly.objects.values_list('id','name','bug').order_by('id')
    fly_np_array = np.array(list(flys))

    # B - fly in lake - 10%
    for x in fly_np_array:
        print (x)
        flys_B = Log.objects.filter(lake = LAKE, fly = x[0]).count()
        print (flys_B)
